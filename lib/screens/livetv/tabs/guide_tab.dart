...
class GuideTab extends StatefulWidget {
  ...
  final bool backReturnsImmediately;
  ...
  const GuideTab({
    ...
    this.backReturnsImmediately = false,
    this.onNavigateUp,
    this.onBack,
  });
...
@visibleForTesting
double liveTvGuideRowHeightFor(double availableGridHeight) => (availableGridHeight / 5.5).clamp(48.0, 52.0);
...
class GuideTabState extends State<GuideTab>
  ...
  double _slotWidth = 156.0;
  double _channelColumnWidth = 112.0;
  double _rowHeight = 50.0;
  static const _sourceHeaderRowHeight = 26.0;
  static const _timeHeaderHeight = 32.0;
  static const _timeNavigationHeight = 36.0;
  ...
  LiveTvProgram? _lastReportedFocusedProgram;
  bool _pendingFocus = false;
  bool _contentFocusInitialized = false;
  ...
  void focusContent() {
    ...
    if (_contentFocusInitialized) {
      _publishFocusSnapshot();
      return;
    }
    _contentFocusInitialized = true;
    _updateFocus(() {
      if (widget.channels.isNotEmpty) {
        _focusZone = _GuideZone.grid;
        _gridColumn = 0;
        _gridChannelIndex = _playingChannelIndex() ?? _gridChannelIndex.clamp(0, widget.channels.length - 1);
  ...
  void jumpToChannel(LiveTvChannel channel) {
    ...
    _updateFocus(() {
      ...
      _gridColumn = 0;
      _focusedProgram = _findCurrentProgram(index);
    });
  ...
  int? _playingChannelIndex() {
    final playingScope = widget.playingChannelScopeKey;
    if (playingScope == null) return null;
    for (var i = 0; i < widget.channels.length; i++) {
      if (liveTvChannelScopeKey(widget.channels[i]) == playingScope) return i;
    }
    return null;
  }
  ...
  void _publishFocusSnapshot() {
    ...
    final reported = _focusedProgram;
    if (!identical(reported, _lastReportedFocusedProgram)) {
      _lastReportedFocusedProgram = reported;
      widget.onProgramFocused?.call(reported);
    }
  }
  ...
  List<_GuideRow> get _guideRows {
    ...
    // Keep source identity inside each compact channel cell. Dedicated source
    // rows used to consume one of the five guide choices at 720p and became
    // especially expensive with several DVRs.
    return [
  ...
  String? _sourceLabelForChannel(LiveTvChannel channel) {
    final groups = groupLiveTvChannelsBySource(widget.channels);
    if (groups.length <= 1) return null;
    return groups.where((group) => group.channels.contains(channel)).firstOrNull?.label;
  }
  ...
  void _tuneChannel(LiveTvChannel channel) {
    final callback = widget.onTuneChannel;
    unawaited(callback != null ? callback(channel) : tuneChannel(channel));
  }
  ...
  KeyEventResult _handleKeyEvent(FocusNode _, KeyEvent event) {
    ...
    if (key.isBackKey) {
      ...
      if (!widget.backReturnsImmediately && _focusZone == _GuideZone.grid) {
  ...
  Widget _buildGuideGrid(ThemeData theme) {
    return LayoutBuilder(
      builder: (context, constraints) {
        final gridHeight = (constraints.maxHeight - _timeNavigationHeight - _timeHeaderHeight).clamp(
          0.0,
          double.infinity,
        );
        _rowHeight = liveTvGuideRowHeightFor(gridHeight);
        _slotWidth = (constraints.maxWidth / 8).clamp(150.0, 160.0);
        _channelColumnWidth = (constraints.maxWidth * 0.09).clamp(108.0, 116.0);
        final rows = _guideRows;
        return Column(
          children: [
            SizedBox(height: _timeNavigationHeight, child: _buildTimeNavigation(theme)),
            Expanded(
    ...
    return _GuideFocusSelector(
      ...
      builder: (context, isFocused) {
        return _ChannelCell(
          ...
          sourceLabel: _sourceLabelForChannel(channel),
          isPlaying: widget.playingChannelScopeKey == liveTvChannelScopeKey(channel),
