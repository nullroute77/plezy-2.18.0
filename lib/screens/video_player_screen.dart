...
import 'livetv/live_tv_guide_layout.dart';
import 'livetv/tabs/guide_tab.dart';
...
class VideoPlayerScreenState extends State<VideoPlayerScreen> with WidgetsBindingObserver, MountedSetStateMixin {
  ...
  LiveTvViewMode _liveTvViewMode = LiveTvViewMode.fullscreen;
  final GlobalKey<GuideTabState> _playbackGuideKey = GlobalKey<GuideTabState>();
  final GlobalKey _liveVideoSurfaceKey = GlobalKey(debugLabel: 'live_video_surface');

  @visibleForTesting
  LiveTvViewMode get liveTvViewMode => _liveTvViewMode;

  @visibleForTesting
  void showLiveTvGuideForTesting() => _showLiveTvGuide();

  @visibleForTesting
  void hideLiveTvGuideForTesting() => _hideLiveTvGuide();

  @visibleForTesting
  Object get liveTvSessionStateIdentityForTesting => _live;

  @visibleForTesting
  Future<void> handleLiveTvBackForTesting() => _handleBackButton();

  @visibleForTesting
  Future<void> switchLiveTvChannelForTesting(LiveTvChannel channel) => _switchLiveChannelTo(channel);

  @visibleForTesting
  int get liveTvChannelIndexForTesting => _live.channelIndex;

  @visibleForTesting
  void adoptLiveTvSessionForTesting(LiveTvPlaybackSession session) => _live.adoptSession(session);

  @visibleForTesting
  Future<void> sendStoppedLiveTvProgressForTesting() => _sendStoppedProgressOnce();
  ...
  late final CompanionRemoteBinding _companionRemote = CompanionRemoteBinding(
    ...
    onStop: () => widget.isLive ? _exitPlayerRoute(navigateHome: false) : _handleBackButton(),
  ...
  Future<void> _handleBackButton({bool navigateHome = false}) async {
    ...
    if (!navigateHome && widget.isLive) {
      if (_liveTvViewMode == LiveTvViewMode.fullscreen) {
        _showLiveTvGuide();
      } else {
        _hideLiveTvGuide();
      }
  ...
  void _handleScreenPlayerNavigation(PlayerNavigationKey navigationKey) {
    ...
    if (widget.isLive && navigationKey == PlayerNavigationKey.back) {
